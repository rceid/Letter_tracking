...
class LetterAdmin(admin.ModelAdmin, ExportCsv):
    list_display = ('title', 'tema', 'tema_específico','date', 'descripción', 'favorable_a_MX', 'mención_directa_a_MX', 
                    'destinatario', 'cámara', 'partido', 'caucus', 'legislatura', 'num_reps', 'num_sens', 'patrocinador_sen',
                    'patrocinador_rep', 'cosign_sorted', 'letter_path', 'observaciones', 'acción', 'notice')
    list_filter = ('legislatura','acción', 'caucus', 'tema', 
                    'tema_específico', 'destinatario', StateLetterFilter)
